...
> It is **self-contained** so it can be referenced from a fresh session in the _new website repo_.
...
- **Primary (Chromium):** keep the `FileSystemFileHandle` from `window.showOpenFilePicker()` when the `.db` was opened, and `createWritable()` to write the edited bytes back to the _same file_.
...
1. **Identifier matching is the riskiest part.** Three different identifier semantics exist (void ElementId int, void GUID, host UniqueId) and the IFC's `GlobalId` is a _derived_ 22-char IfcGuid, not the raw Revit UniqueId. Resolve against a **real `.db` + matching IFC pair early**; keep the geometry-render fallback (#6.4) so the feature degrades gracefully.
